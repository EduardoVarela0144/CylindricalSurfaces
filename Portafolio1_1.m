...
h=title('Grafico de cilindro circular en coordenadas rectangulares $z =+- \sqrt{10-y.^2}$',...
    'interpreter','latex',...
    'FontSize',14) 
view(130,20)
...
x = r.*cos(theta);
y = r.*sin(theta);
z1 = real(+sqrt (10-r.^2.*sin(theta).^2));
z2 = real(-sqrt (10-r.^2.*sin(theta).^2));
...
h=title('Grafico de cilindro circular en coordenadas cilindricas $z = +- \sqrt {10-r.^2*sin(theta).^2}$',...
    'interpreter','latex',...
    'FontSize',14)
view(130,20)
%coloco etiquetas
